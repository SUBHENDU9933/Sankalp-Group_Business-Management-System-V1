...
const ProtectedRoute = () => {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-stone-100"><div className="label-uppercase animate-pulse" data-testid="auth-loading">Loading…</div></div>;
...
const AdminOnly = ({ children }) => {
  ...
  if (loading) return null;
  if (!profile) return <div className="min-h-screen flex items-center justify-center bg-stone-100"><div className="label-uppercase animate-pulse">Loading…</div></div>;
    ...
    <div className="App">
      <ThemeProvider><AuthProvider><BrowserRouter><Routes>
        ...
        <Route element={<ProtectedRoute />}>
          <Route element={<PermissionRoute resource="receipts" />}><Route path="/receipts/:id/print" element={<ReceiptPrintPage />} /></Route>
          <Route element={<PermissionRoute resource="agreements" />}><Route path="/agreements/:id/print" element={<AgreementPrintPage />} /></Route>
          <Route element={<DashboardLayout />}>
            ...
            <Route element={<PermissionRoute resource="estimates" />}><Route path="/estimates" element={<EstimatesPage />} /></Route>
            ...
            <Route element={<PermissionRoute resource="digital_approvals" />}><Route path="/digital-approvals" element={<DigitalApprovalsPage />} /></Route>
            <Route element={<PermissionRoute resource="agreements" />}><Route path="/agreements" element={<AgreementsPage />} /><Route path="/agreements/:id/edit" element={<PermissionRoute resource="agreements" action="edit"><AgreementEditorPage /></PermissionRoute>} /></Route>
      ...
      </Routes></BrowserRouter><Toaster position="bottom-right" duration={2200} /></AuthProvider></ThemeProvider>
    </div>
  );
}
