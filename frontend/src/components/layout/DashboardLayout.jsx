...
import { LayoutDashboard, Users, UserCheck, ReceiptText, Hammer, Truck, ShieldCheck, LogOut, UsersRound, Calculator, FileCheck2, Trash2, Activity, Sun, Moon, FileSignature, Send, BarChart3 } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 grid lg:grid-cols-[260px_1fr]">
      <aside className="shell-dark-scope bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col" data-testid="sidebar">
        ...
        <div className="border-t border-slate-100 dark:border-slate-800 p-3"><NavLink to="/profile" data-testid="sidebar-profile-link" className={({ isActive }) => cn("flex items-center gap-3 p-2 rounded-lg transition-colors", isActive ? "bg-blue-50 dark:bg-blue-950 ring-1 ring-blue-200 dark:ring-blue-800" : "bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700")}><div className="w-9 h-9 rounded-full bg-blue-700 text-white grid place-items-center font-bold text-sm">{(profile?.full_name || profile?.email || "?").slice(0,1).toUpperCase()}</div><div className="flex-1 min-w-0"><div className="text-sm font-semibold text-slate-900 dark:text-slate-100 truncate" data-testid="sidebar-user-name">{profile?.full_name || profile?.email?.split("@")[0] || "—"}</div><div className="inline-flex items-center gap-1.5 mt-0.5"><span className={cn("inline-block w-1.5 h-1.5 rounded-full", isAdmin ? "bg-emerald-500" : "bg-slate-400")} /><span className="text-[10px] tracking-widest uppercase font-semibold text-slate-500 dark:text-slate-400" data-testid="sidebar-user-role">{profile?.designation || ({admin:"Administrator",rm:"Relationship Manager",re:"Relationship Executive"}[role] || "Team")}</span></div></div></NavLink><button onClick={handleLogout} data-testid="sidebar-logout-button" className="w-full mt-2 flex items-center gap-2 px-3 py-2 text-sm text-slate-600 dark:text-slate-300 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950 rounded-lg transition-colors font-medium"><LogOut className="w-4 h-4" /> Sign out</button></div>
      </aside>
      <main className="min-h-screen bg-slate-50 dark:bg-slate-950"><header className="shell-dark-scope bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 lg:px-10 py-3.5 flex items-center justify-between sticky top-0 z-20"><div><div className="text-xs text-slate-500 dark:text-slate-400">Welcome back,</div><div className="font-display text-base font-bold text-slate-900 dark:text-slate-100">{profile?.full_name?.split(" ")[0] || "there"} <span className="ml-1">👋</span></div></div><div className="flex items-center gap-2"><button onClick={toggleTheme} data-testid="theme-toggle-button" aria-label="Toggle light or dark mode" className="w-9 h-9 grid place-items-center rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">{theme === "dark" ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}</button><NotificationBell /></div></header><Outlet /></main>
    </div>
  );
}
