...
export default function AssigneeManager({ lead, profiles = [], onChanged, variant = "inline", buttonClassName = "" }) {
  ...
  const isRmTeamLead = String(role || "").toLowerCase() === "rm" && (
  ...
  const candidates = useMemo(() => {
    ...
    return profiles.filter((p) => {
      ...
      if (String(role || "").toLowerCase() === "rm" && !isAdmin && (!teamReIds.includes(p.id) || String(p.role || "").toLowerCase() !== "re")) return false;
      if (!search) return true;
      const hay = `${p.full_name || ""} ${p.email || ""}`.toLowerCase();
      return hay.includes(search.toLowerCase());
    });
  }, [profiles, currentList, search, role, isAdmin, teamReIds]);
  ...
  if (variant === "inline") {
    return (
      <div className="space-y-2" data-testid="assignee-manager-inline">
        ...
        {canManage && (isAdmin || String(role || "").toLowerCase() === "rm") && (
          <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild><Button variant="outline" size="sm" className="rounded-none border-stone-300 hover:bg-stone-100 h-8 text-xs"><UserPlus className="w-3.5 h-3.5 mr-1" /> {isAdmin ? "Add Assignee" : "Add Team RE"}</Button></PopoverTrigger>
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button type="button" onClick={(e) => e.stopPropagation()} disabled={!canManage} title={canManage ? "Manage lead assignees" : "Only an authorized manager can manage this lead"} className={cn("p-1.5 hover:bg-stone-100 text-stone-600 hover:text-stone-900 disabled:opacity-30 disabled:cursor-not-allowed", buttonClassName)} data-testid={`lead-assignees-quick-${lead?.id}`}><Users className="w-4 h-4" /></button>
      </PopoverTrigger>
      <PopoverContent align="end" className="rounded-none border-stone-300 w-72 p-0" onClick={(e) => e.stopPropagation()}>
        ...
        {canManage && (isAdmin || String(role || "").toLowerCase() === "rm") ? picker : <div className="p-3 text-xs text-stone-500">You don't have permission to manage team assignments on this lead.</div>}
      </PopoverContent>
    </Popover>
  );
}
...
function AssigneePickerList({ candidates, search, onSearchChange, onPick, busy, role, isAdmin }) {
  const label = isAdmin ? "Search assignees…" : String(role || "").toLowerCase() === "rm" ? "Search team REs…" : "Search assignees…";
  return (
    <>
