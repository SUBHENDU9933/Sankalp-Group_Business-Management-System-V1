...
export default function LeadsPage() {
  ...
  const toggleAll = (checked, list) => setSelected(() => (
    checked ? new Set(list.map((l) => l.id)) : new Set()
  ));
  ...
  const filtered = useMemo(() => leads.filter((l) => {
    ...
    if (search) {
      ...
      const hay = [l.name, l.phone, l.phone_secondary, l.location, l.area, l.pincode]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
  ...
  const handleConvert = async (lead) => {
    if (!can("customers", "create")) {
      toast.error("You do not have permission to convert leads to customers");
      return;
    }
  ...
  const handleCancelDelete = async (lead) => {
    if (!can("leads", "delete")) {
      toast.error("You do not have permission to cancel this delete request");
      return;
    }
    try {
  ...
  const runBulk = async (actionLabel, payload) => {
    if (!can("leads", "edit")) {
      toast.error("You do not have permission to bulk edit leads");
      return;
    }
  ...
  const handleBulkAddCoAssignee = async (userId) => {
    if (!can("leads", "assign")) {
      toast.error("You do not have permission to co-assign leads");
      return;
    }
    const ids = selectedIds();
  ...
  return (
    <div data-testid="leads-page">
      ...
      <PageBody>
        ...
        <div className="mt-5">
          {loading ? (
            <div className="bg-white border border-stone-200 p-12 text-center text-sm text-stone-500">
              Loading leads…
            </div>
          ) : filtered.length === 0 ? (
            <div className="bg-white border border-stone-200 p-12 text-center" data-testid="leads-empty">
              ...
              <p className="text-sm text-stone-500 mt-2">
                {leads.length === 0 ? "Add your first lead to start tracking enquiries." : "Try clearing filters or changing the search query."}
              </p>
