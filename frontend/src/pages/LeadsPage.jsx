...
export default function LeadsPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [d, p, paidMap] = await Promise.all([
        fetchLeads(),
        fetchProfiles().catch(() => []),
        fetchLeadPaymentTotals().catch(() => ({})),
      ]);
  ...
  const filtered = useMemo(() => leads.filter((l) => {
    ...
    if (rmFilter !== "all") {
      if (rmFilter === "unassigned") {
        ...
        // User-wise filtering must represent the complete assignment team.
        // A lead can be assigned through either the primary assigned_to column
        // or lead_assignees (the latter is how fresh-lead co-assignment works).
        const matchesPrimary = l.assigned_to === rmFilter;
        const matchesCoAssignee = Array.isArray(l.assignees) && l.assignees.some(
          (a) => (a?.user_id || a?.profile?.id) === rmFilter
        );
        if (!matchesPrimary && !matchesCoAssignee) return false;
      }
    }
  ...
  const handleConvert = async (lead) => {
    if (!can("customers", "create")) { toast.error("You do not have permission to convert leads to customers"); return; }
    if (lead.is_locked || lead.status === "converted") { toast.info("This lead is already converted"); return; }
    if (!window.confirm(`Convert "${lead.name}" to a customer? This will lock the lead.`)) return;
    try { await convertLeadToCustomer(lead, user.id); toast.success("Converted to customer"); load(); nav("/customers"); }
  ...
  const handleRequestDelete = async (lead) => {
    ...
    try { await requestDelete(lead.id, user.id); toast.success("Delete request sent for admin approval"); load(); }
    catch (e) { toast.error(e.message); }
  };
  const handleCancelDelete = async (lead) => {
    if (!can("leads", "delete")) { toast.error("You do not have permission to cancel this delete request"); return; }
  ...
  const runBulk = async (actionLabel, payload) => {
    ...
    try { const n = await bulkUpdateLeads(ids, payload); toast.success(`Updated ${n} lead${n !== 1 ? "s" : ""}`); clearSelection(); load(); }
    catch (e) { toast.error(e.message); }
  };
  ...
  const handleBulkAddCoAssignee = async (userId) => {
    ...
    try { await bulkAddCoAssignee(ids, userId, user.id); toast.success(`Added ${label} to ${ids.length} lead${ids.length !== 1 ? "s" : ""}`); clearSelection(); load(); }
    catch (e) { toast.error(e.message); }
  };
  ...
  const handleExportSelected = () => {
    ...
    const rows = selectedLeads(); if (!rows.length) return;
    exportLeadsCSV(rows, `leads-selected-${new Date().toISOString().slice(0, 10)}.csv`); toast.success(`Exported ${rows.length} leads`);
  };
  const handleExportFiltered = () => {
    if (!can("leads", "view")) return;
    if (!filtered.length) { toast.info("No leads to export"); return; }
    exportLeadsCSV(filtered, `leads-${new Date().toISOString().slice(0, 10)}.csv`); toast.success(`Exported ${filtered.length} leads`);
  };
      ...
      <PageHeader subtitle="Phase 2" title="Lead Management" actions={(
        <>
          {canView && <DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline" className="rounded-none border-stone-300 hover:bg-stone-100" data-testid="lead-bulk-menu-btn"><FileSpreadsheet className="w-4 h-4 mr-1" />Bulk<ChevronDown className="w-3 h-3 ml-1" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="rounded-none border-stone-300 w-56">
            {canCreate && <DropdownMenuItem className="rounded-none cursor-pointer" onClick={() => setImportOpen(true)} data-testid="lead-import-open"><Upload className="w-4 h-4 mr-2" />Import from CSV…</DropdownMenuItem>}
            <DropdownMenuItem className="rounded-none cursor-pointer" onClick={downloadLeadTemplate} data-testid="lead-template-download"><Download className="w-4 h-4 mr-2" />Download CSV template</DropdownMenuItem>
            <DropdownMenuSeparator /><DropdownMenuItem className="rounded-none cursor-pointer" onClick={handleExportFiltered} data-testid="lead-export-filtered"><Download className="w-4 h-4 mr-2" />Export filtered ({filtered.length})</DropdownMenuItem>
      ...
      <PageBody>
        ...
        <div className="mt-5">
          {loading ? <div className="bg-white border border-stone-200 p-12 text-center text-sm text-stone-500">Loading leads…</div>
            ...
            : view === "table" ? <LeadTableView leads={filtered} onOpen={openDetails} onEdit={openEdit} onStatusChange={handleStatusChange} onConvert={handleConvert} onRequestDelete={handleRequestDelete} onCancelDelete={handleCancelDelete} selected={selected} onToggleSelect={toggleSelect} onToggleAll={(checked) => toggleAll(checked, filtered)} profiles={profiles} onAssigneesChanged={load} />
            : <LeadPipelineView leads={filtered} onOpen={openDetails} onStatusChange={handleStatusChange} onConvert={handleConvert} />}
        </div>
      </PageBody>
