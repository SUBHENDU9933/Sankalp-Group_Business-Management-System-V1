...
export default function LeadsPage() {
  ...
  const handleBulkAddCoAssignee = async (rmId) => { if (!can("leads", "assign")) { toast.error("You do not have permission to assign leads"); return; } const ids = selectedIds(); if (!ids.length || !rmId) return; const rm = profiles.find((p) => p.id === rmId); const label = rm ? (rm.full_name || rm.email) : "selected RM"; if (!window.confirm(`Add ${label} as Co-RM to ${ids.length} lead${ids.length !== 1 ? "s" : ""}?`)) return; try { await bulkAddCoAssignee(ids, rmId, user.id); toast.success(`Added ${label} to ${ids.length} lead${ids.length !== 1 ? "s" : ""}`); clearSelection(); load(); } catch (e) { toast.error(e.message); } };
