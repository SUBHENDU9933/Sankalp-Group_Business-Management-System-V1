...
export default function ReportsPage() {
  ...
  const subtitle = isAdmin
    ...
    : isRM
      ...
      : "Your permitted business scope across leads, projects, collections, and lead sources";
    ...
    <>
      ...
      <PageBody className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          {isAdmin && <Stat label="Total Spent" value={formatINR(totals.spent)} tone="rose" />}
          <Stat label="Outstanding" value={formatINR(totals.outstanding)} tone="amber" />
        </div>

        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          <div className="label-uppercase mb-3">{isAdmin ? "Collections vs Expenses — Last 12 Months" : "Collections — Last 12 Months"}</div>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={monthlyTrend}>
              ...
              {isAdmin && <Bar dataKey="expenses" name="Expenses" fill="#F97316" radius={[4, 4, 0, 0]} />}
            </BarChart>
          </ResponsiveContainer>
        </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="text-left text-xs text-slate-400 uppercase border-b border-slate-100"><th className="pb-2">Owner</th><th className="pb-2 text-right">Projects</th><th className="pb-2 text-right">Contract Value</th><th className="pb-2 text-right">Collected</th>{isAdmin && <><th className="pb-2 text-right">Spent</th><th className="pb-2 text-right">Cash Margin</th></>}</tr></thead>
                <tbody>
                  {rmRows.map((r) => <tr key={r.rmId} className="border-b border-slate-50"><td className="py-2 font-medium text-slate-800">{r.rmName}</td><td className="py-2 text-right">{r.projects}</td><td className="py-2 text-right">{formatINR(r.contractValue)}</td><td className="py-2 text-right text-emerald-700">{formatINR(r.collected)}</td>{isAdmin && <><td className="py-2 text-right text-rose-700">{formatINR(r.spent)}</td><td className="py-2 text-right font-semibold">{formatINR(r.netPL)}</td></>}</tr>)}
                  {rmRows.length === 0 && <tr><td colSpan={isAdmin ? 6 : 4} className="py-6 text-center text-slate-400">No project data in this scope.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
