...
function buildAgreementCover(containerEl) {
  const source = (containerEl?.innerText || "").replace(/\u00a0/g, " ");
  const title = firstMatch(source, /This\s+(.+?)\s+\("Agreement"\)/i, "Interior Work Agreement");
  const agreementRef = firstMatch(source, /Agreement Ref:\s*([^·\n]+)/i);
  const generated = firstMatch(source, /Generated\s+([^\n]+)/i);
  const clientName = firstMatch(source, /Mr\.\/Ms\.\s+([^,\n]+?)(?:,\s*[^\n]*?)?\s+residing at/i);
  const mobile = firstMatch(source, /Mobile No\.\s*([+\d][\d\s-]{7,})/i);
  const estimateNo = firstMatch(source, /Estimate No\.\s*([^,;\n]+)/i);
  const estimateDate = firstMatch(source, /Estimate No\.\s*[^\n]*?dated\s+([0-9]+\s*\/\s*[0-9]+\s*\/\s*[0-9]+)/i);
  const category = firstMatch(source, /Final Selected Category:\s*([^\.\n]+)/i);
  const value = firstMatch(source, /Final Agreed Contract Value:\s*₹?([\d,]+)/i);
  const projectLocation = firstMatch(source, /for the Client's Interior Project at\s*([^\n]+?)(?:,\s*under Estimate|\s+under Estimate)/i);
  const timeline = firstMatch(source, /approximately\s+([^\.]+working days)/i);

  const schedule = [];
  const scheduleMatch = source.match(/Schedule\s*[—-]\s*([^\n]+)/i);
  if (scheduleMatch) {
    scheduleMatch[1].split("·").forEach((part) => {
      const m = part.trim().match(/(.+?)\s*:\s*(\d+)%/);
      if (m) schedule.push({ stage: m[1].trim(), percent: `${m[2]}%` });
    });
  }
  ...
  ]).map((r, i) => `<div style="flex:1;text-align:center;padding:0 7px;${i ? "border-left:1px solid #cbd5e1" : ""}"><div style="margin:auto;width:38px;height:38px;border-radius:50%;background:#f5a623;color:#0b2b55;font-size:16px;font-weight:800;line-height:38px">${i + 1}</div><div style="font-size:21px;font-weight:800;margin-top:6px">${r.percent}</div><div style="font-size:10px;font-weight:700;line-height:1.3">${r.stage}</div></div>`).join("");
  ...
  cover.innerHTML = `
    ...
    <div style="border:1px solid #cbd5e1;border-radius:15px;background:#f8fafc;padding:13px 16px;margin-top:13px;display:flex;align-items:center;gap:16px"><div style="font-size:11px;font-weight:800;letter-spacing:1px;color:#123d82;width:140px">◷ &nbsp; PROJECT TIMELINE</div><div style="border-left:1px solid #e58b19;padding-left:16px;font-size:12px"><span style="font-weight:700">Estimated Execution</span><br><span style="font-size:19px;font-weight:800;color:#123d82">${timeline}</span></div></div>
...
/** Export a premium dynamic cover followed by the existing agreement pages. */
